...
class ThumbnailGeneratorSpec extends FlatSpec with BeforeAndAfterAll with Matchers {
  ...
  "Object Thumbnail Generator generateThumbnail" should "add the thumbnail to ObjectData" in {
    ...
    val metadata = Map("identifier" -> "do_123", "appIcon" -> "https://sunbirddev.blob.core.windows.net/sunbird-content-dev/content/assets/do_11400964389868339217/indiagate.jpeg", "IL_UNIQUE_ID" -> "do_123", "objectType" -> "QuestionSet", "name" -> "Test QuestionSet", "status" -> "Live")
