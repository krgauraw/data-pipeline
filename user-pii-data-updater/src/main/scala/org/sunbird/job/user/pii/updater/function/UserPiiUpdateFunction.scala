...
class UserPiiUpdateFunction(config: UserPiiUpdaterConfig, httpUtil: HttpUtil,
  ...
  override def processElement(userEvent: UserPiiEvent, context: ProcessFunction[UserPiiEvent, String]#Context, metrics: Metrics): Unit = {
    ...
    logger.info(s"UserPiiUpdateFunction event: ${userEvent} | requestId: ${requestId}")
    try {
      ...
      targetObjectTypes.foreach(entry => {
        ...
        schemaVersions.foreach(ver => {
          ...
          userPiiFields.foreach(pii => {
            ...
            logger.info(s"UserPiiUpdateFunction :: Processing for pii update : userId: ${userEvent.userId} , objectType : ${entry._1} , schemaVersion : ${ver} , pii_field_config: ${pii}, requestId: ${requestId}")
            val nodes: List[ObjectData] = searchObjects(entry._1, pii._1, ver, userEvent.userId, pii._2.asInstanceOf[List[String]])(neo4JUtil)
            if (!nodes.isEmpty) {
              nodes.map(node => {
                logger.info(s"UserPiiUpdateFunction ::: processing node with metadata ::: ${node.metadata} | requestId: ${requestId}")
                ...
                logger.info(s"UserPiiUpdateFunction ::: metadata going to be updated for ${node.id} ::: ${meta} | requestId: ${requestId}")
              ...
              logger.info(s"No Object Found For objectType: ${entry._1}, userId: ${userEvent.userId}, lookupKey: ${pii._1} | requestId: ${requestId}")
            }
          })
        })
      })
