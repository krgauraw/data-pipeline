...
class OwnershipTransferFunction(config: UserPiiUpdaterConfig, httpUtil: HttpUtil,
  ...
  override def processElement(event: OwnershipTransferEvent, context: ProcessFunction[OwnershipTransferEvent, String]#Context, metrics: Metrics): Unit = {
    ...
    val featureName = event.eventContext.getOrElse("featureName", "").asInstanceOf[String]
    logger.info(s"Feature: ${featureName} | OwnershipTransferFunction event: ${event} | requestId: ${requestId}")
    try {
      ...
      if (isValidUserRole(toUserRoles)(config)) {
        ...
        if (null != event.assetInfo.isEmpty && !event.assetInfo.isEmpty) {
          ...
          logger.info(s"Feature: ${featureName} | OwnershipTransferFunction :: Processing objectType : ${objectType} , Identifier : ${assetId} for ownership transfer between :: from userId: ${event.fromUserId} , to userId: ${event.toUserId} | requestId: ${requestId}")
          ...
          if (null != node) {
            ...
            userPiiFields.foreach(pii => {
              ...
              if (StringUtils.isNotBlank(updatedId)) {
                logger.info(s"Feature: ${featureName} | Node Updated Successfully for identifier: ${nodeId} | requestId: ${requestId}")
                ...
                logger.info(s"Feature: ${featureName} | Node Update Failed for identifier: ${nodeId} | requestId: ${requestId}")
                if (StringUtils.equalsIgnoreCase("Default", node.getOrElse("visibility", "").asInstanceOf[String]))
                  failedIdMap.put(nodeId.replace(".img", ""), node.getOrElse("status", "").asInstanceOf[String])
              }
            })
          } else {
            val exitMsg = s"Feature: ${featureName} | No Object Found with Identifier ${assetId}, objectType: ${objectType} having ownership  with user id : ${event.fromUserId}"
          ...
          targetObjectTypes.foreach(entry => {
            ...
            schemaVersions.foreach(ver => {
              ...
              val toUserName = event.toUserProfile.getOrElse("firstName", "").asInstanceOf[String] + " " + event.toUserProfile.getOrElse("lastName", "").asInstanceOf[String]
              userPiiFields.foreach(pii => {
                logger.info(s"Feature: ${featureName} | OwnershipTransferFunction :: Processing objectType : ${entry._1} , schemaVersion : ${ver} for ownership transfer between :: from userId: ${event.fromUserId} , to userId: ${event.toUserId} | requestId: ${requestId}")
                val nodes: List[ObjectData] = searchObjects(entry._1, pii._1, ver, event.fromUserId)(neo4JUtil)
                if (!nodes.isEmpty) {
                  logger.info(s"Feature: ${featureName} | OwnershipTransferFunction ::: ${nodes.size} nodes found for ownership transfer. | requestId: ${requestId}")
                  nodes.map(node => {
                    logger.info(s"Feature: ${featureName} | OwnershipTransferFunction ::: processing node with metadata ::: ${node.metadata} | requestId: ${requestId}")
                    val meta: Map[String, AnyRef] = Map(pii._1 -> event.toUserId, pii._2.asInstanceOf[List[String]].head -> toUserName)
                    logger.info(s"Feature: ${featureName} | OwnershipTransferFunction ::: metadata going to be updated for ${node.id} ::: ${meta} | requestId: ${requestId}")
                  ...
                  logger.info(s"Feature: ${featureName} | No Object Found For objectType: ${entry._1}, userId: ${event.fromUserId}, lookupKey: ${pii._1} | requestId: ${requestId}")
                }
              })
            })
          })
        }
        processOwnershipTransferResult(event, idMap, failedIdMap, featureName)(config, httpUtil, metrics)
      } else {
        val exitMsg = s"Feature: ${featureName} | To User does not have any valid roles. | toUser Roles: ${toUserRoles} | Expected Roles: ${config.ownershipTransferValidRoles.toList}"
        logger.info(LoggerUtil.getExitLogs(config.jobName, requestId, exitMsg))
      }
    } catch {
      case e: Throwable => {
