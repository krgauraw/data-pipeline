...
trait UserPiiUpdater extends NotificationProcessor {
  ...
  def processResult(userEvent: UserPiiEvent, idMap: util.HashMap[String, String], failedIdMap: util.HashMap[String, String], featureId: String)(implicit config: UserPiiUpdaterConfig, httpUtil: HttpUtil, metrics: Metrics): Unit = {
    val requestId = userEvent.eventContext.getOrElse("requestId", "").asInstanceOf[String]
    if (!idMap.isEmpty && failedIdMap.isEmpty) {
      logger.info(s"Feature: ${featureId} | UserPiiUpdater ::: processResult :: All PII data processed successfully for user id : ${userEvent.userId}. Total Identifiers affected : ${idMap.keySet()} | requestId: ${requestId}")
      ...
      val exitMsg = s"Feature: ${featureId} | UserPiiUpdater ::: processResult :: Event Skipped for user id : ${userEvent.userId} because no object found for given user."
      ...
      val exitMsg = s"Feature: ${featureId} | UserPiiUpdater ::: processResult :: All PII data processing completed partially for user id : ${userEvent.userId}. Total Success Identifiers: ${idMap.keySet()} | Total Failed Identifiers : ${failedIdMap.keySet()}"
      logger.info(LoggerUtil.getExitLogs(config.jobName, requestId, exitMsg))
      ...
      val exitMsg = s"Feature: ${featureId} | UserPiiUpdater ::: processResult :: All PII data processing failed for user id : ${userEvent.userId}. Total Failed Identifiers : ${failedIdMap.keySet()}"
      logger.info(LoggerUtil.getExitLogs(config.jobName, requestId, exitMsg))
  ...
  def processOwnershipTransferResult(event: OwnershipTransferEvent, idMap: util.HashMap[String, String], failedIdMap: util.HashMap[String, String], featureId: String)(implicit config: UserPiiUpdaterConfig, httpUtil: HttpUtil, metrics: Metrics): Unit = {
    val requestId = event.eventContext.getOrElse("requestId", "").asInstanceOf[String]
    if (!idMap.isEmpty && failedIdMap.isEmpty) {
      val exitMsg = s"Feature: ${featureId} | UserPiiUpdater ::: processOwnershipTransferResult :: All data transferred to ${event.toUserId} successfully. Total Identifiers affected : ${idMap.keySet()}"
      logger.info(LoggerUtil.getExitLogs(config.jobName, requestId, exitMsg))
      metrics.incCounter(config.ownershipTransferSuccessEventCount)
    } else if (idMap.isEmpty && failedIdMap.isEmpty) {
      val exitMsg = s"Feature: ${featureId} | UserPiiUpdater ::: processOwnershipTransferResult :: Event Skipped with from userId : ${event.fromUserId} because no object found for given user."
      logger.info(LoggerUtil.getExitLogs(config.jobName, requestId, exitMsg))
      metrics.incCounter(config.ownershipTransferSkippedEventCount)
    } else if (!idMap.isEmpty && !failedIdMap.isEmpty) {
      val exitMsg = s"Feature: ${featureId} | UserPiiUpdater ::: processOwnershipTransferResult :: Ownership Transfer processing completed partially for from user id : ${event.fromUserId}. Total Success Identifiers: ${idMap.keySet()} | Total Failed Identifiers : ${failedIdMap.keySet()}"
      logger.info(LoggerUtil.getExitLogs(config.jobName, requestId, exitMsg))
      ...
      val exitMsg = s"Feature: ${featureId} | UserPiiUpdater ::: processOwnershipTransferResult :: Ownership Transfer processing failed between from_user_id : ${event.fromUserId} to_user_id : ${event.toUserId}. Total Failed Identifiers : ${failedIdMap.keySet()}"
      logger.info(LoggerUtil.getExitLogs(config.jobName, requestId, exitMsg))
